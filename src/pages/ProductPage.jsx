...
export default function ProductPage() {
  ...
  // Nayi state sirf Add To Cart ke button loading ke liye
  ...
  const addToCartHandler = async () => {
    ...
    try {
      // Assuming addToCart is an async function in your CartContext
      await addToCart(id); 
    } catch (error) {
      console.error(error);
    } finally {
                  ...
                  {isAuth ? (
                    <>
                      {product.stock <= 0 ? (
                        ...
                        // ✅ Naya Add To Cart Button loader ke sath
                        <motion.button
                          whileHover={{ scale: 0.98 }}
                          whileTap={{ scale: 0.96 }}
                          onClick={addToCartHandler}
                          disabled={cartBtnLoading}
                          className={`flex h-16 w-full items-center justify-center gap-4 text-sm uppercase tracking-[4px] text-white transition-all duration-300 ${
                            cartBtnLoading 
                              ? "bg-gray-500 cursor-not-allowed" 
                              : "bg-[#1d1d1d] hover:bg-[#BEA163] hover:text-black cursor-pointer"
                          }`}
                        >
                          {cartBtnLoading ? (
